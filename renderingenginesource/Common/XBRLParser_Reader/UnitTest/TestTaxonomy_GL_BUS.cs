...
namespace Aucent.MAX.AXE.XBRLParser.Test_Taxonomy
{
	...
	/// <exclude/>
	[TestFixture] 
	public class TestTaxonomy_GL_BUS : Taxonomy
	{
		#region Overrides
...
//		}
		...
		string BUS_FILE = TestCommon.FolderRoot + @"gl" +System.IO.Path.DirectorySeparatorChar +"bus" +System.IO.Path.DirectorySeparatorChar +"gl-bus.xsd";
		...
		/// <exclude/>
		[Test]
		[Ignore("Requires advanced tuple support")]
		public void LoadAndParse()
		{
			//Trace.Listeners.Add( new TextWriterTraceListener(Console.Out) );

			TestTaxonomy_GL_BUS s = new TestTaxonomy_GL_BUS();

			int errors = 0;

			if ( s.Load( BUS_FILE, out errors ) != true )
			{
