	...
	public class ErrorWriter : StringWriter
	{
		...
		private void SaveFailedFiles()
		{
			...
			if( this._baseReport != null )
			{
				...
				this._baseReport.SaveAsXml( basePath );
			}
			...
			if( this._generatedReport != null )
			{
				...
				this._generatedReport.SaveAsXml( genPath );
			}

			XsltArgumentList args = new XsltArgumentList();
			args.AddParam( "asPage", string.Empty, "false" );
			args.AddParam( "showFlags", string.Empty, "true" );
            args.AddParam( "numberDecimalSeparator", string.Empty, Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator );
            args.AddParam( "numberGroupSeparator", string.Empty, Thread.CurrentThread.CurrentCulture.NumberFormat.NumberGroupSeparator );
            args.AddParam( "numberGroupSize", string.Empty, Thread.CurrentThread.CurrentCulture.NumberFormat.NumberGroupSizes[ 0 ].ToString() ); 
