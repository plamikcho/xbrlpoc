...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="InstanceReport">
		<!-- if the columns or cells have FootnoteIndexer -->
		<xsl:variable name="hasFootnotes" select="count( Columns/Column[ not( '' = FootnoteIndexer ) ] ) + count( Rows/Row/Cells/Cell[ not( '' = FootnoteIndexer ) ] ) &gt; 0" />
	...
	-->
					...
					<!-- if this report is not embeddedm write the footnotes on the inside -->
					<xsl:if test="$isOuterReport">
				...
				<!-- if this report is embeddedm write the footnotes on the outside -->
				...
				<xsl:if test="$isOuterReport">
					<div style="display: none;">
						<xsl:for-each select="//Row[ string-length( ElementName ) &gt; 0 and not( ./ElementName = following::Row/ElementName ) ]">
	...
	<xsl:template name="authRefData">
		...
		<table border="0" cellpadding="0" cellspacing="0" class="authRefData" style="display: none;">
			<xsl:attribute name="id">defref_<xsl:value-of select="ElementName" /></xsl:attribute>
			<tr>
				<td class="hide">
	...
	<xsl:template name="numFilters">
		...
		<!-- output the value -->
		<xsl:if test="NumericAmount &lt; 0">(</xsl:if>
		<xsl:choose>
			<xsl:when test="IsRatio = 'true'">
				<!-- <xsl:variable name="decimal" select="substring( substring-after( $numeric, '.' ), 3 )" /> -->
				<xsl:variable name="number" select="number( $numeric )" />
				<!-- <xsl:variable name="number" select="number( $numeric ) * 100" /> -->
				<xsl:variable name="absolute" select="$number*( 1 - 2*( $number &lt; 0 ) )" />
				<!-- <xsl:variable name="whole" select="floor( $absolute )" /> -->

				<xsl:value-of select="format-number( $absolute, '0.00DDD%', 'currency' )" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="decimal" select="substring-after( $numeric, '.' )" />
				<xsl:variable name="number" select="number( $numeric )" />
				<xsl:variable name="absolute" select="$number*( 1 - 2*( $number &lt; 0 ) )" />
				<xsl:variable name="whole" select="floor( $absolute )" />
