...
\s*$", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnorePatternWhitespace );
		...
		public ReportBuilder()
			: this(null, RulesEngineUtils.DefaultRulesFile, ReportFormat.Xml, HtmlReportFormat.None )
		{
		}
		...
		/// </param>
		public ReportBuilder(UpdateStatusExecutor executor, string rulesFileNameIn, ReportFormat type, HtmlReportFormat htmlFormat )
		{
            // plamen
		...
		private void LoadCurrencies()
		{
			...
			if( File.Exists( currencyCodeFile ) )
			{
				...
				{
					Trace.TraceError( "Error: Loading of the Currency Mapping File caused the filing error:" + separator + ex.Message );
				}
			}
			else
			{
				Trace.TraceWarning( "Warning: The Currency Mapping File specified could not be found." + separator +
		...
		public string GetDocumentationInformation( string elementID, string language )
		{
			...
			{
				executor.Invoke( ex.TargetSite.ToString() + ": " + ex.ToString() );
					...
					foreach( Cell cell in embededCells )
					{
						...
						if( cell.EmbeddedReport.ProcessEmbedCommands( baseReport, this.currentFilingSummary ) )
						{
							...
							cell.EmbeddedReport.InstanceReport.ReportName =
								embededReportHeader.ShortName + separator + cell.EmbeddedReport.InstanceReport.ReportName;
